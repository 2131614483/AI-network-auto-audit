...
PLUGIN_DIR = declaration_dir("aiops.postmortem-draft")
...
def test_protocol_is_schema_valid_and_stays_a_read_only_contract() -> None:
    ...
    assert protocol["id"] == "aiops.postmortem-draft"
    ...
    assert capability["id"] == "aiops.postmortem.draft"
    assert capability["side_effects"] == "read_only"
    assert capability["policy"]["approval_required"] is True
    assert capability["inputs"][0]["contract_id"] == "incident-proposal"
    assert capability["outputs"][0]["contract_id"] == "postmortem-draft"
...
def test_manifest_is_schema_valid_and_declares_no_write_permission() -> None:
    ...
    assert manifest["id"] == "aiops.postmortem-draft"
    assert manifest["capabilities"] == ["aiops.postmortem.draft"]
...
def test_postmortem_draft_contract_accepts_positive_sample_and_rejects_negative_sample() -> None:
    schema = _schema("postmortem-draft.schema.json")
    Draft202012Validator.check_schema(schema)
    validator = Draft202012Validator(schema)

    positive = _load(CONTRACT_DIR / "postmortem-draft@1.positive.json")
    validator.validate(positive)

    negative = _load(CONTRACT_DIR / "postmortem-draft@1.negative.json")
    messages = [error.message for error in validator.iter_errors(negative)]
    assert messages
    assert any("'draft' was expected" in message for message in messages)
    assert any("'urgency'" in message for message in messages)
