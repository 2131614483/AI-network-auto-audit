...
PLUGIN_DIR = declaration_dir("knowledge.entity-relation-candidate")
...
def test_protocol_is_schema_valid_and_stays_a_read_only_contract() -> None:
    ...
    assert protocol["id"] == "knowledge.entity-relation-candidate"
    ...
    assert capability["id"] == "knowledge.extract.relations"
    assert capability["side_effects"] == "read_only"
    assert capability["inputs"][0]["contract_id"] == "document-content"
    assert capability["outputs"][0]["contract_id"] == "graph-candidate-set"
...
def test_graph_candidate_set_rejects_undeclared_relation_types() -> None:
    schema = _schema("graph-candidate-set.schema.json")
    validator = Draft202012Validator(schema)

    negative = _load(CONTRACT_DIR / "graph-candidate-set@1.negative.json")
    assert any(
        "owns_everything" in error.message for error in validator.iter_errors(negative)
    )
