...
PLUGIN_DIR = declaration_dir("audit.workpaper-export")
...
def test_protocol_is_schema_valid_and_stays_a_read_only_contract() -> None:
    ...
    assert protocol["id"] == "audit.workpaper-export"
    ...
    assert capability["id"] == "audit.workpaper.export"
    ...
    assert capability["outputs"][0]["contract_id"] == "workpaper-export"
...
def test_manifest_is_schema_valid_and_declares_no_write_permission() -> None:
    ...
    assert manifest["id"] == "audit.workpaper-export"
    assert manifest["capabilities"] == ["audit.workpaper.export"]
...
def test_workpaper_export_contract_accepts_positive_sample_and_rejects_negative_sample() -> None:
    schema = _schema("workpaper-export.schema.json")
    Draft202012Validator.check_schema(schema)
    validator = Draft202012Validator(schema)

    positive = _load(CONTRACT_DIR / "workpaper-export@1.positive.json")
    validator.validate(positive)

    negative = _load(CONTRACT_DIR / "workpaper-export@1.negative.json")
    messages = [error.message for error in validator.iter_errors(negative)]
    assert messages
