...
PLUGIN_DIR = declaration_dir("knowledge.retention-recommendation")
...
def test_protocol_is_schema_valid_and_stays_a_read_only_contract() -> None:
    ...
    assert protocol["id"] == "knowledge.retention-recommendation"
    ...
    assert capability["id"] == "knowledge.retention.recommend"
    assert capability["side_effects"] == "read_only"
    assert capability["policy"]["approval_required"] is True
    assert capability["inputs"][0]["contract_id"] == "document-content"
    assert capability["outputs"][0]["contract_id"] == "retention-recommendation"
...
def test_manifest_is_schema_valid_and_declares_no_write_permission() -> None:
    ...
    assert manifest["id"] == "knowledge.retention-recommendation"
    assert manifest["capabilities"] == ["knowledge.retention.recommend"]
    assert manifest["side_effects"] == "read_only"
    assert manifest["permissions"]["data_write"] == []
    assert manifest["permissions"]["network"] == "none"


def test_document_content_contract_accepts_positive_sample_and_rejects_negative_sample() -> None:
    schema = _schema("document-content.schema.json")
    Draft202012Validator.check_schema(schema)
    validator = Draft202012Validator(schema, registry=_registry())

    positive = _load(CONTRACT_DIR / "document-content@1.positive.json")
    validator.validate(positive)

    negative = _load(CONTRACT_DIR / "document-content@1.negative.json")
    messages = [error.message for error in validator.iter_errors(negative)]
    assert messages
    assert any("'klingon'" in message for message in messages)


def test_retention_recommendation_contract_accepts_positive_sample_and_rejects_negative_sample() -> None:
    schema = _schema("retention-recommendation.schema.json")
    Draft202012Validator.check_schema(schema)
    validator = Draft202012Validator(schema)

    positive = _load(CONTRACT_DIR / "retention-recommendation@1.positive.json")
    validator.validate(positive)

    negative = _load(CONTRACT_DIR / "retention-recommendation@1.negative.json")
    messages = [error.message for error in validator.iter_errors(negative)]
    assert messages
    assert any("'proposed' was expected" in message for message in messages)
    assert any("'delete_now'" in message for message in messages)
