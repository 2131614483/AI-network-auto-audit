...
type EvolutionRing = {
  ...
  evidence: Record<string, number>; detail: string; checked: string | null;
};
            ...
            return {
              ...
              description: (
                <Space direction="vertical" size={2} style={{ width: "100%" }}>
                  ...
                  {evidence.length ? (
                    <Typography.Text>实测数字：{evidence.map(([key, value]) => `${key} = ${fmtInt(value)}`).join(" · ")}</Typography.Text>
                  ...
                  {ring.checked ? <Typography.Text type="secondary">校验时间：{new Date(ring.checked).toLocaleString("zh-CN")}</Typography.Text> : null}
                </Space>
              ),
            };
          })} />
