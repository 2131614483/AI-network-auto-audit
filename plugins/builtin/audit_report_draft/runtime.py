"""Isolated implementation for the verified read-only audit report-draft plugin.

R3 受审批输出：只读已确认 Finding 集合，按固定模板版本生成确定性、可追溯的
报告草稿。报告签发仍由人完成；草稿永不覆盖历史报告，落库由治理服务在人工
审批后以独立身份完成。
"""
...
def handle(envelope: dict[str, Any]) -> dict[str, Any]:
    ...
    if envelope.get("plugin_id") != "audit.report-draft":
        raise InputRejected("unexpected plugin identity")
    if envelope.get("capability") != "audit.report.draft":
        raise InputRejected("unexpected capability")
    report_input = envelope.get("report")
    if not isinstance(report_input, dict):
        raise InputRejected("report input is missing")
    artifact = report_input.get("finding_set")
    ...
    template_version = report_input.get("template_version", DEFAULT_TEMPLATE_VERSION)
    if not isinstance(template_version, str) or not template_version.strip():
        raise InputRejected("template_version is missing")
    ...
    truncated = len(findings) > MAX_FINDINGS_LIMIT
    included = findings[:MAX_FINDINGS_LIMIT]

    finding_ids = [finding["finding_id"] for finding in included]
    report_seed = "|".join([actual_hash, *sorted(finding_ids), template_version])
    report_id = hashlib.sha256(report_seed.encode("utf-8")).hexdigest()[:16]
    ...
    executive_summary = (
        f"本报告草稿基于 {len(included)} 项已确认发现生成，"
        f"其中高风险 {severity_counts['high']} 项、中风险 {severity_counts['medium']} 项、"
        f"低风险 {severity_counts['low']} 项；签发前须完成人工复核。"
    )
    conclusion = "建议管理层针对已确认发现补充控制措施并跟踪整改，正式签发由授权人员完成。"
    ...
    return {
        ...
        "provenance": {
            ...
            "template_version": template_version,
            "plugin": "audit.report-draft@0.1.0",
