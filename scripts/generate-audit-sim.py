...
Run: python scripts/generate-audit-sim.py

The output under ``.data/audit-sim/`` is committed (it is a deterministic test
*input*, not runtime state), so tests do not depend on this having been run.
"""
...
# Derived from this file, never hardcoded: the previous revision pinned an
# absolute Windows path, so the generator only worked on one machine — which
# is also why CI and fresh clones had no fixtures at all.
...
def _ledger_rows() -> list[dict]:
    ...
    for i in range(1, 81):
        # `acct_credit` was computed and never used; dropping it changes nothing
        # about the generated data (verified byte-identical) and keeps ruff clean.
        acct_debit = ACCOUNTS[i % len(ACCOUNTS)]
